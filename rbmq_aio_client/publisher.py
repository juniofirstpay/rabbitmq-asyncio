...
class Publisher:
    ...
    def __run_blocking(self, connection, exchange):
        existing_loop = False
        try:
            loop = asyncio.get_running_loop()
            existing_loop = True
    ...
    def mark_daemon(self):
        self.__is_daemon = True
        
    def run(self, connection: str, exchange: str, queue_size=1000000):
        if self.__is_daemon:
            self.__queue = queue.Queue(maxsize=queue_size)    
            self.__run_daemon_thread(connection, exchange)
        else:
            self.__run_blocking(connection, exchange)
